...
@section('content')
    <form class="space-y-6" action="{{ route('projects.tasks.store', $project) }}" method="POST">
        ...
        <div>
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300" for="title">Task Title</label>
            <input
        ...
        <div>
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300" for="deadline">Deadline</label>
            <input
                class="mt-1 block w-full rounded-md border-gray-300 p-2 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200"
                id="deadline" name="deadline" type="date" value="{{ old('deadline') }}" required>
            @error('deadline')
                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="flex justify-end space-x-4">
            <a class="rounded-md bg-gray-200 px-4 py-2 text-gray-800 hover:bg-gray-300 dark:bg-gray-600 dark:text-gray-200 dark:hover:bg-gray-500"
                href="{{ route('projects.show', $project) }}">Cancel</a>
            <button class="rounded-md bg-indigo-600 px-4 py-2 text-white hover:bg-indigo-700" type="submit">Save
                Task</button>
        </div>
    </form>
@endsection
